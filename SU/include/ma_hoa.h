...
static const unsigned char KHOA_GOC_AES[16] =
{
    0x2B, 0x7E, 0x15, 0x16,
    0x28, 0xAE, 0xD2, 0xA6,
    0xAB, 0xF7, 0x15, 0x88,
...
inline bool Tao_Khoa_Phien_AES(
    uint8_t khoa_phien[16],
    uint64_t session_id)
{
    uint8_t thong_diep[16] =
    {
        'A', 'E', 'S', '-', 'S', 'E', 'S', 'S',
        (uint8_t)(session_id >> 56),
        (uint8_t)(session_id >> 48),
        (uint8_t)(session_id >> 40),
    ...
    uint32_t nonce_seq)
{
    ...
    if (!Tao_Khoa_Phien_AES(khoa_phien, session_id))
    {
        mbedtls_gcm_free(&gcm);
        return false;
    }

    int ket_qua = mbedtls_gcm_setkey(
        &gcm,
    ...
    ket_qua = mbedtls_gcm_crypt_and_tag(
        &gcm,
        MBEDTLS_GCM_ENCRYPT,
        payload_len,
        iv,
    ...
    uint32_t nonce_seq)
{
    ...
    ket_qua = mbedtls_gcm_auth_decrypt(
            &gcm,
            payload_len,
            iv,
            AES_GCM_IV_LEN,
